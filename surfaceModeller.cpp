...
typedef struct Quad
{
	...
	Vector3D normal; //start from the back
} Quad;

// Quads
Quad *qarray;
GLboolean quadArrayAllocated = false;

// Vertices 
#define NUMBEROFSIDES 16 // You may want to lower this to 4 or 5 when debugging

Vertex *varray; // 
GLboolean varrayAllocated = false;
...
static void createVBO(){
    ...
    size_t vSize = sizeof (float) * subcurve.numCurvePoints * NUMBEROFSIDES * 3;
    size_t nSize = sizeof (float) * subcurve.numCurvePoints * NUMBEROFSIDES * 3;

    size_t iSize = sizeof (unsigned int) * (subcurve.numCurvePoints * NUMBEROFSIDES * 4);

    // copy vertex attribs data to VBO
    glBindBuffer(GL_ARRAY_BUFFER, vboId);
    glBufferData(GL_ARRAY_BUFFER, vSize+nSize, 0, GL_STATIC_DRAW); // reserve space
    ...
    glDrawElements(GL_QUADS,            // primitive type
                   iSize,               // # of indices
